...
```
RECAST/
├── RECAST/                     # Python package (model, training, drift score)
...
```bash
git clone https://github.com/labiip/RECAST.git
cd RECAST
pip install -e .
```

**Core dependencies:** Python ≥3.9, PyTorch, scanpy, anndata, scikit-learn, scipy, pandas, tqdm (installed via `pip install -e .`).
